...
categorySelect.addEventListener("change", function () {
  //evento para cambiar la manera de ordenar los productos
...
function getProducts(page, sortBy) {
  if (urlParams.has("cat")) {
    //si la url tiene el parametro cat
...
function createListView(products) {
  ...
  products.forEach((product) => {
    ...
    let addToFavorites = productItem.querySelector(
      `#add-to-favorites-${product.id}`
    );
...
function createTableView(products) {
  ...
  products.forEach((product) => {
    ...
    let addToFavorites = productRow.querySelector(
      `#add-to-favorites-${product.id}`
    );
    addFavoriteEventListeners(addToFavorites, product.id);
...
function displayView(currentView) {
  getProducts(currentPage, sortBy).then((products) => {
    if (currentView === "list") {
