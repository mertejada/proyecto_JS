...
function addCartEventListeners(cartItem, productId) {
  cartItem.addEventListener("submit", (event) => {
    ...
    if (currentUser) {
      ...
      if (units < 1) {
        //lo hago aqui aunque ya lo hago en el html
      ...
      getProductInfo(productId)
        .then((product) => {
          let userCart =
            JSON.parse(localStorage.getItem(`cart-${currentUser}`)) || []; //consigo el carrito del usuario o un array vacio
    ...
    if (currentUser) {
      getProductInfo(productId)
        .then((product) => {
          ...
          let existingProduct = userFavorites.find(
